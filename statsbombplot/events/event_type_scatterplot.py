...
def draw_event_type_scatter(events, filename, event_type, draw_mean_height=True, marker_shape='o', marker_color=(0.765, 0.388, 0.961, 0.8)):
    ...
    for i, event in events.iterrows():
            ...
            ax.scatter(x,y, s=markersize, edgecolor='black', linewidth=linewidth, facecolor=marker_color, zorder=7, marker=marker_shape)


    legend_elements = [
        plt.scatter(x,y, s=markersize, edgecolor='black', linewidth=linewidth, facecolor=marker_color, zorder=7, marker=marker_shape, label=event_type)
    ]
    ...
    if draw_mean_height:
        ...
        legend_elements.extend([
             mlines.Line2D([], [], color=(0.88, 0.48, 0.37, 0.8), linewidth=2, linestyle='dashed', label=f'Mean Height of {event_type}')
        ])
    ...
    ax.text(0, -2.1, f'Number of events: {len(events)}', fontsize=7, va='center', ha='left')
    ax.text(0, -4.1, f'Percentage in opponent half: {percentage_opponent_half}%', fontsize=7, va='center', ha='left')
